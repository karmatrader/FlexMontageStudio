...
def process_voice_and_proxy(channel_name):
    ...
    def parse_arguments():
        ...
        parser.add_argument('--voice_speed', type=float, default=default_voice_speed)
        parser.add_argument('--voice_style', type=str, default=default_voice_style)
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        if response_data.get("detail", {}).get("status") == "detected_unusual_activity":
                            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Обнаружена необычная активность = BAN IP. Ожидание 2 минуты...")
                            time.sleep(120)
                            continue
                            ...
                            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Не удалось сгенерировать аудио: {response.text}")
                            return False
                    except ValueError:
                        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Не удалось распознать ответ сервера: {response.text}")
                        return False
            except requests.exceptions.Timeout:
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Тайм-аут соединения. Повтор через 10 секунд...")
                if attempt < max_retries - 1:
                    time.sleep(10)
                else:
                    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Превышено количество попыток из-за тайм-аутов.")
                    return False
            except (requests.exceptions.ProxyError, requests.exceptions.ConnectionError) as e:
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Ошибка соединения. Повтор через 10 секунд...")
                if attempt < max_retries - 1:
                    time.sleep(10)
                else:
                    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Превышено количество попыток.")
                    return False
        ...
        try:
            ...
            for index, row in df.iloc[current_row:].iterrows():
                ...
                if text and text.lower() != 'nan':
                    file_number = str(index + 1).zfill(3) if (index + 1) < 100 else str(index + 1)
                    output_file_path = os.path.join(OUTPUT_DIRECTORY, f"{file_number}.mp3")

                    if os.path.exists(output_file_path):
                        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Файл {output_file_path} уже существует, пропуск строки {index + 1}")
                        continue
